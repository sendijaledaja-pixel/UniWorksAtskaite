...
const NAVY="#0d2137", NAVY2="#163352", ORNG="#e8521a", RED="#dc2626";
...
const emptyForm=()=>({darbinieki:[],kategorijas:[],selectedObjekts:null,objekts:"",datums:getToday(),laiks_no:"",laiks_lidz:"",statuss:"",darba_apraksts:"",avarija:false,nakts:false,materiali:false,materiali_saraksts:[emptyMat()],pieziimes:"",foto:[]});

function calcHours(no,lidz){
  if(!no||!lidz)return null;
  const[h1,m1]=no.split(":").map(Number),[h2,m2]=lidz.split(":").map(Number);
...
function BuildingBg(){
  ...
  return(
    <svg width="100%" height="100%" viewBox={`0 0 ${tw+40} ${th+60}`} preserveAspectRatio="xMidYMid slice"
      ...
      {Array.from({length:rows},(_,row)=>Array.from({length:cols},(_,col)=>{
        ...
        return(
          <g key={idx}>
            <rect x={x} y={y} width={W} height={H} rx="3" fill={on?"#fbbf24":"white"} opacity={on?0.85:0.08} style={{transition:"all 0.9s"}}/>
            {on&&<><rect x={x+W/2-0.5} y={y} width="1" height={H} fill="rgba(0,0,0,0.15)"/><rect x={x} y={y+H/2-0.5} width={W} height="1" fill="rgba(0,0,0,0.12)"/></>}
          </g>
        );
      }))}
...
export default function App(){
  ...
  useEffect(()=>{store.get("uw_reports").then(v=>{if(v)setReports(v);});},[]);

  async function saveReports(list){await store.set("uw_reports",list);setReports(list);}
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  const toggleWorker=name=>{const c=form.darbinieki;set("darbinieki",c.includes(name)?c.filter(d=>d!==name):[...c,name]);};
  const toggleKat=id=>{const c=form.kategorijas;set("kategorijas",c.includes(id)?c.filter(k=>k!==id):[...c,id]);};
  const updateMat=(i,k,v)=>{const a=[...form.materiali_saraksts];a[i]={...a[i],[k]:v};set("materiali_saraksts",a);};

  function selectObj(o){
    if(o){setForm(f=>({...f,selectedObjekts:o,objekts:o.adrese}));}
    else{setForm(f=>({...f,selectedObjekts:null,objekts:""}));}
  ...
  async function submit(){
    ...
    await saveReports([r,...reports]);
    const ok=await sendTelegram(r);
    setTgOk(ok);setSending(false);setDone(true);
  }

  function newForm(av=false){setForm({...emptyForm(),avarija:av,nakts:av?isNight():false});setDone(false);setTgOk(null);setView("form");}
  ...
  const filtered=OBJEKTI.filter(o=>o.nos.toLowerCase().includes(q.toLowerCase())||o.adrese.toLowerCase().includes(q.toLowerCase()));
  ...
  const NavBar=({title,sub,back,red})=>(
    <div style={{background:red?`linear-gradient(135deg,${RED},#991b1b)`:`linear-gradient(135deg,${NAVY},${NAVY2})`,padding:"14px 20px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:20,boxShadow:"0 2px 12px rgba(13,33,55,0.2)"}}>
      ...
      {!back&&<Logo/>}
      {back&&<Logo/>}
      <div style={{flex:1}}><div style={{color:"#fff",fontSize:15,fontWeight:700,lineHeight:1.3}}>{title}</div>{sub&&<div style={{color:"rgba(255,255,255,0.45)",fontSize:12}}>{sub}</div>}</div>
    </div>
  );
  ...
  if(showModal)return(
    <div style={{position:"fixed",inset:0,zIndex:50,display:"flex",flexDirection:"column",background:"#f1f5f9",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      ...
      <div style={{padding:"12px 16px",background:"#fff",borderBottom:"1px solid #e2e8f0",flexShrink:0}}>
        <input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder="🔍 Meklēt pēc nosaukuma vai adreses..." style={{...inp,marginBottom:0}}/>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"8px 16px 24px"}}>
        ...
        <div onClick={()=>selectObj(null)} style={{background:"#f8fafc",border:"1.5px dashed #e2e8f0",borderRadius:14,padding:"14px 16px",marginTop:4,cursor:"pointer",display:"flex",alignItems:"center",gap:12}}>
          ...
          <div><div style={{fontWeight:600,fontSize:14,color:"#475569",marginBottom:2}}>Cits objekts</div><div style={{fontSize:12,color:"#94a3b8"}}>Ievadīt manuāli</div></div>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      ...
      <div style={{padding:"0 16px 40px",marginTop:-16}}>
        <div onClick={()=>newForm(false)} style={{background:`linear-gradient(135deg,${ORNG},#ff6b35)`,borderRadius:20,padding:"20px 22px",marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:16,boxShadow:`0 10px 30px rgba(232,82,26,0.4)`}}>
          ...
          <div style={{flex:1}}><div style={{fontWeight:800,fontSize:18,color:"#fff",letterSpacing:"-0.02em",marginBottom:2}}>Jauna atskaite</div><div style={{fontSize:13,color:"rgba(255,255,255,0.7)"}}>Standarta darba pārskats</div></div>
        ...
        <div onClick={()=>newForm(true)} style={{background:"#fff",border:"2px solid #fca5a5",borderRadius:20,padding:"18px 22px",marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:16,boxShadow:"0 4px 16px rgba(220,38,38,0.10)"}}>
          ...
          <div style={{flex:1}}><div style={{fontWeight:800,fontSize:17,color:RED,letterSpacing:"-0.02em",marginBottom:2}}>Avārijas izsaukums</div><div style={{fontSize:13,color:"#ef4444",opacity:0.7}}>{isNight()?"🌙 Nakts izsaukums aktīvs":"Ārkārtas situācija objektā"}</div></div>
          <div style={{color:"#fca5a5",fontSize:24}}>›</div>
        </div>
        <div onClick={()=>setView("reports")} style={{background:"#fff",border:"1px solid #eaecf0",borderRadius:20,padding:"18px 22px",marginBottom:12,cursor:"pointer",display:"flex",alignItems:"center",gap:16}}>
          ...
          <div style={{flex:1}}><div style={{fontWeight:700,fontSize:16,color:NAVY,marginBottom:2}}>Atskaišu vēsture</div><div style={{fontSize:13,color:"#94a3b8"}}>{reports.length} ieraksti kopā</div></div>
    ...
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"system-ui,-apple-system,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{textAlign:"center",maxWidth:340,width:"100%"}}>
          <div style={{width:80,height:80,borderRadius:"50%",background:form.avarija?"#fef2f2":"#f0fdf4",border:`3px solid ${form.avarija?"#fca5a5":"#86efac"}`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px",fontSize:36}}>{form.avarija?"🚨":"✓"}</div>
          <div style={{fontSize:22,fontWeight:800,color:NAVY,marginBottom:6}}>{form.avarija?"Avārija reģistrēta!":"Paldies, "+form.darbinieki[0]+"!"}</div>
          {form.selectedObjekts?(
            <div style={{marginBottom:16}}><div style={{fontSize:15,fontWeight:600,color:NAVY}}>{form.selectedObjekts.nos}</div><div style={{fontSize:13,color:"#64748b"}}>{form.selectedObjekts.adrese}</div></div>
          ):<div style={{fontSize:15,color:"#64748b",marginBottom:16}}>{form.objekts}</div>}
          <div style={{fontSize:13,color:"#16a34a",marginBottom:16,background:"#f0fdf4",border:"1px solid #86efac",borderRadius:10,padding:"10px 14px",display:"flex",alignItems:"center",gap:8,justifyContent:"center"}}><span>✈️</span><span>Nosūtīts uz Telegram ✓</span></div>
  ...
  if(view==="reports")return(
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"system-ui,-apple-system,sans-serif",color:NAVY}}>
      ...
      <div style={{padding:16,maxWidth:480,margin:"0 auto",paddingBottom:40}}>
        {reports.length===0&&<div style={{textAlign:"center",color:"#94a3b8",padding:"80px 20px"}}><div style={{fontSize:48,marginBottom:16}}>📭</div><div style={{fontSize:16,fontWeight:600}}>Vēl nav atskaišu</div><div style={{fontSize:13,marginTop:4}}>Iesniedz pirmo atskaiti</div></div>}
        {reports.map(r=>{
          ...
          return(
            <div key={r.id} style={{...card,border:r.avarija?"1.5px solid #fca5a5":"1px solid #eaecf0"}}>
              ...
              <div style={{fontSize:13,color:"#64748b",marginBottom:8,display:"flex",flexWrap:"wrap",gap:"4px 10px"}}>
                <span>👷 {(r.darbinieki||[]).join(" & ")}</span><span>📅 {r.datums}</span>{h&&<span>⏱ {h}</span>}
              </div>
              ...
              {mats.length>0&&<div style={{borderTop:"1px solid #f1f5f9",paddingTop:10}}>{mats.map((m,i)=><div key={i} style={{fontSize:13,color:"#475569",marginBottom:4,display:"flex",gap:6}}><span style={{color:ORNG,fontSize:10,marginTop:4}}>●</span><span>{m.nosaukums}{m.veikals?" · "+m.veikals:""}{m.cena&&<strong style={{color:NAVY}}> · €{m.cena}</strong>}</span></div>)}</div>}
      ...
      <div style={{padding:16,maxWidth:480,margin:"0 auto",paddingBottom:40}}>
        ...
        <div style={{...card,border:form.avarija?`1.5px solid #fca5a5`:"1px solid #eaecf0",background:form.avarija?"#fef2f2":"#fff"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:form.avarija?14:0}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div><div style={{fontWeight:700,fontSize:15,color:form.avarija?RED:NAVY}}>Avārijas izsaukums</div><div style={{fontSize:12,color:"#94a3b8",marginTop:1}}>Ārkārtas situācija</div></div>
            </div>
          ...
          {form.avarija&&(
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fff",borderRadius:10,padding:"12px 14px",border:"1.5px solid #fca5a5"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div><div style={{fontWeight:600,fontSize:14,color:form.nakts?RED:NAVY}}>Nakts izsaukums</div><div style={{fontSize:12,color:"#94a3b8"}}>21:00 – 07:00</div></div>
              </div>
        ...
        <div style={card}>
          ...
          <div style={{display:"flex",gap:8}}>
            {DARBINIEKI.map(d=>{const a=form.darbinieki.includes(d);return(<button key={d} onClick={()=>toggleWorker(d)} style={{flex:1,padding:"14px 8px",borderRadius:12,border:a?`2px solid ${ORNG}`:"1.5px solid #e2e8f0",background:a?`${ORNG}0a`:"#f8fafc",color:a?ORNG:NAVY,fontWeight:a?700:400,fontSize:16,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>{d}</button>);})}
          </div>
        ...
        <div style={card}>
          ...
          {form.selectedObjekts?(
            <div style={{background:`${ORNG}08`,border:`1.5px solid ${ORNG}`,borderRadius:12,padding:"14px 16px",marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
              ...
              <button onClick={()=>setShowModal(true)} style={{background:"rgba(0,0,0,0.04)",border:"none",color:"#94a3b8",fontSize:12,cursor:"pointer",fontFamily:"inherit",flexShrink:0,padding:"4px 10px",borderRadius:6}}>Mainīt</button>
            </div>
          ):(
            <button onClick={()=>setShowModal(true)} style={{width:"100%",background:"#f8fafc",border:"1.5px dashed #e2e8f0",borderRadius:12,padding:"16px",marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:12,fontFamily:"inherit"}}>
              ...
              <div style={{flex:1,textAlign:"left"}}><div style={{fontWeight:600,fontSize:14,color:NAVY,marginBottom:1}}>Izvēlēties no saraksta</div><div style={{fontSize:12,color:"#94a3b8"}}>{OBJEKTI.length} objekti pieejami</div></div>
          ...
          <input style={{...inp,marginBottom:0,background:form.selectedObjekts?"#f0fdf4":"#f8fafc",borderColor:form.selectedObjekts?"#86efac":"#e2e8f0"}} placeholder="Adrese vai apraksts..." value={form.objekts} onChange={e=>{set("objekts",e.target.value);if(form.selectedObjekts)set("selectedObjekts",null);}}/>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {KATEGORIJAS.map(k=>{const a=form.kategorijas.includes(k.id);return(<button key={k.id} onClick={()=>toggleKat(k.id)} style={{display:"flex",alignItems:"center",gap:8,padding:"11px 12px",borderRadius:10,border:a?`2px solid ${ORNG}`:"1.5px solid #e2e8f0",background:a?`${ORNG}08`:"#f8fafc",color:a?ORNG:NAVY,fontWeight:a?600:400,fontSize:13,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s",textAlign:"left"}}><span style={{fontSize:17}}>{k.icon}</span>{k.label}</button>);})}
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <textarea style={{...inp,minHeight:90,resize:"vertical",marginBottom:0}} placeholder="Apraksti detalizēti, kādi darbi tika veikti..." value={form.darba_apraksts} onChange={e=>set("darba_apraksts",e.target.value)}/>
        </div>
        ...
        <div style={card}>
          ...
          {uploading&&<div style={{fontSize:13,color:"#94a3b8",textAlign:"center",padding:"8px 0"}}>Apstrādā...</div>}
          {form.foto.length>0&&<><div style={{fontSize:13,color:"#64748b",marginBottom:10,fontWeight:500}}>{form.foto.length} foto pievienoti</div><PhotoGrid photos={form.foto} onRemove={i=>set("foto",form.foto.filter((_,j)=>j!==i))}/></>}
        </div>
        ...
        <div style={card}>
          ...
          <textarea style={{...inp,minHeight:72,resize:"vertical",marginBottom:0}} placeholder="Jebkas cits, ko vēlies piebilst..." value={form.pieziimes} onChange={e=>set("pieziimes",e.target.value)}/>
        </div>
        ...
        <button onClick={canSubmit&&!sending?submit:undefined} style={{width:"100%",background:!canSubmit||sending?"#e2e8f0":form.avarija?`linear-gradient(135deg,${RED},#b91c1c)`:`linear-gradient(135deg,${ORNG},#ff6b35)`,border:"none",borderRadius:14,padding:18,color:canSubmit&&!sending?"#fff":"#94a3b8",fontSize:17,fontWeight:800,cursor:canSubmit&&!sending?"pointer":"not-allowed",fontFamily:"inherit",letterSpacing:"-0.01em",boxShadow:canSubmit&&!sending?(form.avarija?"0 8px 24px rgba(220,38,38,0.4)":"0 8px 24px rgba(232,82,26,0.35)"):"none",transition:"all 0.2s"}}>
